...
def main():
    ...
    cur = conn2.cursor()
    c.execute("""select item_id, pic_url from share_taobaoke_item where item_id=1 order by item_id asc """)
    w_1st = 500
    w_2nd = 200
    st_size = 450,500
    nd_size = 200,200
...
def UpdateUUID():
    conn = sqlite3.connect("db\Taoke.db")
    c = conn.cursor()
    c.execute("""Update share_taobaoke_item Set pic_width_detail=null, pic_height_detail=null, pic_url_detail=null
                            ,pic_width_grid=null
