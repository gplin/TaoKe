...
def RequestItemData():
    ...
    for a.keyword in ['女装','男装','数码','家居','字画','电器','上衣']:
        print a.keyword
        f= a.getResponse()
        if f.has_key("taobaoke_items_get_response"):
           res = f['taobaoke_items_get_response']
           total_results = res["total_results"]
           #Data =[{},{}]
           data = res["taobaoke_items"]["taobaoke_item"]
           SaveToDB(data)
...
def SaveToDB(data):
    ...
    for d in data:
        ...
        c.execute("""Insert Into share_Taobaoke_Item
        (user_id, category_id,add_up_love,add_up_bookmark,add_up_comment,
         volume, nick, click_url, commission_rate,
         commission_num,price,seller_credit_score,
         num_iid,item_location,commission,shop_click_url,
         commission_volume,
         title,
         pic_url,
         share_date,uuid,
...
def downLoad_pic(pic_url):
    try:
        ...
        #checking dir
        path = ["../media/img/o", "../media/img/g", "../media/img/d"]
        for p in path:
        ...
        return [path_o[3:],g_size[0],g_size[1],path_g[3:],d_size[0],d_size[1],path_d[3:]]
    except Exception,e:
        print "error:",(e)
        pass
        return []
