(function() {
	...
	if (p) {
		p = ["hasOwnProperty", "valueOf", "isPrototypeOf",
	...
	Array.from = function(i) {
		...
		return (a.isEnumerable(i) && typeof i != "string") ? (o(i) == "array")
				? i
				: n.call(i) : [i]
	};
	...
	d("String", String, ["charAt", "charCodeAt", "concat", "indexOf",
					"lastIndexOf", "match", "quote", "replace", "search",
					"slice", "split", "substr", "substring", "toLowerCase",
					"toUpperCase"])("Array", Array, ["pop", "push", "reverse",
					"shift", "sort", "splice", "unshift", "concat", "join",
					"slice", "indexOf", "lastIndexOf", "filter", "forEach",
					"every", "map", "some", "reduce", "reduceRight"])("Number",
			Number, ["toExponential", "toFixed", "toLocaleString",
					"toPrecision"])("Function", f, ["apply", "call", "bind"])(
			"RegExp", RegExp, ["exec", "test"])("Object", Object, ["create",
...
Array.implement({
			...
			getRandom : function() {
				return (this.length)
						? this[Number.random(0, this.length - 1)]
						: null
			},
			include : function(a) {
				if (!this.contains(a)) {
			...
			flatten : function() {
				...
				for (var b = 0, a = this.length; b < a; b++) {
					...
					d = d.concat((c == "array" || c == "collection"
							|| c == "arguments" || instanceOf(this[b], Array))
...
String.implement({
			test : function(a, b) {
				return ((typeOf(a) == "regexp") ? a : new RegExp("" + a, b))
						.test(this)
			},
			contains : function(a, b) {
...
(function(b) {
	...
	b.each(function(c) {
				if (!Number[c]) {
					a[c] = function() {
						return Math[c].apply(null, [this].concat(Array
										.from(arguments)))
					}
				}
			});
...
Function.implement({
			...
			bind : function(c) {
				var a = this, b = (arguments.length > 1) ? Array.slice(
						arguments, 1) : null;
				return function() {
...
(function() {
	...
	$uid(i);
	$uid(k);
	var a = navigator.userAgent.toLowerCase(), c = navigator.platform
			.toLowerCase(), j = a
			.match(/(opera|ie|firefox|chrome|version)[\s\/:]([\w\d\.]+)?.*?(safari|version[\s\/:]([\w\d\.]+)|$)/)
			|| [null, "unknown", 0], f = j[1] == "ie" && k.documentMode;
	var o = this.Browser = {
		extend : Function.prototype.extend,
		name : (j[1] == "version") ? j[3] : j[1],
		version : f || parseFloat((j[1] == "opera" && j[4]) ? j[4] : j[2]),
		Platform : {
			name : a.match(/ip(?:ad|od|hone)/) ? "ios" : (a
					.match(/(?:webos|android)/)
					|| c.match(/mac|win|linux/) || ["other"])[0]
		},
		Features : {
			xpath : !!(k.evaluate),
			air : !!(i.runtime),
	...
	try {
		...
		Array.from = function(p) {
			if (typeof p != "string" && Type.isEnumerable(p)
					&& typeOf(p) != "array") {
		...
		["pop", "push", "reverse", "shift", "sort", "splice", "unshift",
				"concat", "join", "slice"].each(function(e) {
					var p = l[e];
					Array[e] = function(q) {
...
var Event = new Type("Event", function(a, i) {
	...
	if (n.indexOf("key") != -1) {
		...
		if ((/click|mouse|menu/i).test(n)) {
			o = (!o.compatMode || o.compatMode == "CSS1Compat")
					? o.html
					: o.body;
			m = {
...
(function() {
	var a = this.Class = new Type("Class", function(h) {
				...
				var g = function() {
					...
					this.$caller = null;
					var i = (this.initialize) ? this.initialize.apply(this,
							arguments) : this;
	...
	var c = function() {
		...
		var g = this.$caller.$name, h = this.$caller.$owner.parent, i = (h)
				? h.prototype[g]
				: null;
		if (!i) {
...
(function() {
	this.Chain = new Class({
				...
				callChain : function() {
					return (this.$chain.length) ? this.$chain.shift().apply(
...
(function() {
	...
	var j = new RegExp("^(?:\\s*(,)\\s*|\\s*(<combinator>+)\\s*|(\\s+)|(<unicode>+|\\*)|\\#(<unicode>+)|\\.(<unicode>+)|\\[\\s*(<unicode1>+)(?:\\s*([*^$!~|]?=)(?:\\s*(?:([\"']?)(.*?)\\9)))?\\s*\\](?!\\])|(:+)(<unicode>+)(?:\\((?:(?:([\"'])([^\\13]*)\\13)|((?:\\([^)]+\\)|[^()]*)+))\\))?)"
...
(function() {
	...
	j.isXML = function(c) {
		return (!!c.xmlVersion) || (!!c.xml)
				|| (b.call(c) == "[object XMLDocument]")
	...
	j.setDocument = function(w) {
		...
		if (o.isHTMLDocument) {
			...
			if (z.querySelectorAll) {
				try {
					...
					o.starSelectsClosedQSA = (s && !!s.length && s[0].nodeName
							.charAt(0) == "/")
				} catch (v) {
				}
				try {
			...
			o.nativeMatchesSelector = y.matchesSelector || y.mozMatchesSelector
		...
		o.hasAttribute = (y && this.isNativeCode(y.hasAttribute)) ? function(C,
				B) {
		...
		o.documentSorter = (y.compareDocumentPosition) ? function(C, B) {
			...
			var D = E.ownerDocument.createRange(), B = C.ownerDocument
					.createRange();
			D.setStart(E, 0);
			D.setEnd(E, 0);
			B.setStart(C, 0);
	...
	var e = /^([#.]?)((?:[\w-]+|\*))$/, g = /\[.+[*$^]=(?:""|'')?\]/, f = {};
	j.search = function(q, D, P, v) {
		var B = this.found = (v) ? null : (P || []);
		if (!q) {
			return B
		} else {
			if (q.navigator) {
		...
		var z, O, s = this.uniques = {}, y = !!(P && P.length), c = (q.nodeType == 9);
		...
		if (typeof D == "string") {
			...
			simpleSelectors : if (C) {
				...
				if (!L) {
					...
					if (L == "#") {
						...
						I = q.getElementById(W);
						if (!I) {
							return B
						}
						if (this.idGetsName
								&& I.getAttributeNode("id").nodeValue != W) {
							break simpleSelectors
						}
						if (v) {
							return I || null
						...
						if (L == ".") {
							if (!this.isHTMLDocument
									|| ((!q.getElementsByClassName || this.brokenGEBCN) && q.querySelectorAll)) {
								break simpleSelectors
							}
							if (q.getElementsByClassName && !this.brokenGEBCN) {
								G = q.getElementsByClassName(W);
								if (v) {
									return G[0] || null
								}
								for (O = 0; I = G[O++];) {
			...
			querySelector : if (q.querySelectorAll) {
				if (!this.isHTMLDocument
						...
						|| f[D]
						|| (this.brokenCheckedQSA && D.indexOf(":checked") > -1)
						|| (this.brokenEmptyAttributeQSA && g.test(D))
						|| d.disableQSA) {
					break querySelector
				}
				var A = D;
				if (!c) {
					var N = q.getAttribute("id"), p = "slickid__";
					q.setAttribute("id", p);
					A = "#" + p + " " + A
				}
				try {
		...
		this.posNTHType = {};
		this.posNTHTypeLast = {};
		this.push = (!y && (v || (z.length == 1 && z.expressions[0].length == 1)))
				? this.pushArray
				: this.pushUID;
		if (B == null) {
			B = []
		...
		var M, H, F;
		var J, V, E, U, R, x, t;
		var w, r, o, S, T = z.expressions;
		search : for (O = 0; (r = T[O]); O++) {
			for (M = 0; (o = r[M]); M++) {
				J = "combinator:" + o.combinator;
				if (!this[J]) {
					continue search
				}
				V = (this.isXMLDocument) ? o.tag : o.tag.toUpperCase();
				E = o.id;
				U = o.classList;
				R = o.classes;
				x = o.attributes;
	...
	j.matchNode = function(m, n) {
		if (this.isHTMLDocument && this.nativeMatchesSelector) {
			try {
				return this.nativeMatchesSelector.call(m, n.replace(
								/\[([^=]+)=\s*([^'"\]]+?)\s*\]/g, '[$1="$2"]'))
			} catch (u) {
			}
		}
		...
		for (q = 0; (currentExpression = r[q]); q++) {
			if (currentExpression.length == 1) {
				...
				if (this.matchSelector(m, (this.isXMLDocument) ? o.tag : o.tag
								.toUpperCase(), o.id, o.classes, o.attributes,
						o.pseudos)) {
					return true
				}
				s++
	...
	j.matchSelector = function(n, u, c, o, p, r) {
		if (u) {
			var s = (this.isXMLDocument) ? n.nodeName : n.nodeName
					.toUpperCase();
			if (u == "*") {
				if (s < "@") {
					return false
		...
		if (p) {
			for (q = p.length; q--;) {
				m = p[q];
				if (m.operator ? !m.test(this.getAttribute(n, m.key)) : !this
						.hasAttribute(n, m.key)) {
					return false
				}
			}
		}
		...
		" " : function(p, v, m, q, r, t, o) {
			...
			if (this.isHTMLDocument) {
				getById : if (m) {
					...
					if ((!u && p.all)
							|| (this.idGetsName && u && u
									.getAttributeNode("id").nodeValue != m)) {
						n = p.all[m];
						if (!n) {
							return
						}
				...
				getByClass : if (q && p.getElementsByClassName
						&& !this.brokenGEBCN) {
	...
	var k = {
		...
		"nth-last-child" : j.createNTHPseudo("lastChild", "previousSibling",
				"posNTHLast"),
		"nth-of-type" : j.createNTHPseudo("firstChild", "nextSibling",
				"posNTHType", true),
		"nth-last-of-type" : j.createNTHPseudo("lastChild", "previousSibling",
				"posNTHTypeLast", true),
		index : function(m, c) {
		...
		focus : function(c) {
			return this.isHTMLDocument && this.document.activeElement === c
...
(function() {
	...
	var o = ["defaultValue", "accessKey", "cellPadding", "cellSpacing",
			"colSpan", "frameBorder", "maxLength", "readOnly", "rowSpan",
			"tabIndex", "useMap"];
	var d = ["compact", "nowrap", "ismap", "declare", "noshade", "checked",
			"disabled", "readOnly", "multiple", "selected", "noresize",
			"defer", "defaultChecked"];
	var g = {
		html : "innerHTML",
		"class" : "className",
		"for" : "htmlFor",
	...
	Element.implement({
				...
				get : function(q) {
					...
					return (p && p.get) ? p.get.apply(this) : this
							.getProperty(q)
				}.overloadGetter(),
				erase : function(q) {
					var p = Element.Properties[q];
					(p && p.erase) ? p.erase.apply(this) : this
							.removeProperty(q);
					return this
				},
				...
				getProperty : function(q) {
					q = o[q] || q;
					var p = g[q] || m[q];
					return (p) ? this[p] : (d[q]) ? !!this[q] : (l.test(q)
							? this.getAttribute(q, 2)
							: (p = this.getAttributeNode(q))
									? p.nodeValue
									: null)
							|| null
				},
				...
				removeProperty : function(q) {
					q = o[q] || q;
					var p = g[q];
					(p) ? this[p] = "" : (d[q]) ? this[q] = false : this
							.removeAttribute(q);
					return this
				},
				...
				toQueryString : function() {
					var p = [];
					this.getElements("input, select, textarea").each(
							function(r) {
								var q = r.type;
								if (!r.name || r.disabled || q == "submit"
										|| q == "reset" || q == "file"
										|| q == "image") {
									return
								}
								var s = (r.get("tag") == "select")
										? r.getSelected().map(function(t) {
													return document.id(t)
															.get("value")
												})
										: ((q == "radio" || q == "checkbox") && !r.checked)
												? null
												: r.get("value");
								Array.from(s).each(function(t) {
									if (typeof t != "undefined") {
										p.push(encodeURIComponent(r.name) + "="
	...
	Element.implement("clone", function(r, p) {
				...
				if (r) {
					var s = w.getElementsByTagName("*"), u = this
				...
				a(w, this, p);
				if (Browser.ie) {
					var t = w.getElementsByTagName("object"), v = this
...
(function() {
	...
	Element.implement({
				getComputedStyle : function(h) {
					...
					var g = Element.getDocument(this).defaultView, f = g ? g
							.getComputedStyle(this, null) : null;
					return (f) ? f.getPropertyValue((h == a) ? "float" : h
							.hyphenate()) : null
				},
				...
				setStyle : function(g, f) {
					...
					if (typeOf(f) != "string") {
						...
						f = Array.from(f).map(function(k, j) {
							...
							return (typeOf(k) == "number") ? h[j].replace("@",
									Math.round(k)) : k
						}).join(" ")
					} else {
				...
				getStyle : function(l) {
					...
					if (Browser.opera || (Browser.ie && isNaN(parseFloat(f)))) {
						if ((/^(height|width)$/).test(l)) {
							var g = (l == "width") ? ["left", "right"] : [
									"top", "bottom"], i = 0;
							g.each(function(m) {
										i += this.getStyle("border-" + m
												+ "-width").toInt()
												+ this.getStyle("padding-" + m)
														.toInt()
									}, this);
...
(function() {
	...
	[Element, Window, Document].invoke("implement", {
				...
				removeEvent : function(e, d) {
					...
					return (Element.NativeEvents[e]) ? this.removeListener(e,
							g, arguments[2]) : this
				},
				addEvents : function(b) {
					for (var c in b) {
	...
	var a = function(b) {
		...
		return (c != this && c.prefix != "xul" && typeOf(this) != "document" && !this
				.contains(c))
	};
	Element.Events = {
		mouseenter : {
	...
	Element.implement({
		...
		getOffsets : function() {
			if (this.getBoundingClientRect && !Browser.Platform.ios) {
				var r = this.getBoundingClientRect(), o = document.id(this
						.getDocument().documentElement), q = o.getScroll(), t = this
						.getScrolls(), s = (k(this, "position") == "fixed");
				return {
	...
	var f = this.Fx = new Class({
		...
		step : function(g) {
			if (this.options.frameSkip) {
				var h = (this.time != null) ? (g - this.time) : 0, i = h
						/ this.frameInterval;
				this.time = g;
				this.frame += i
			} else {
...
(function(j, l) {
	...
	var h = function() {
		...
		l.removeListener("DOMContentLoaded", h).removeListener(
				"readystatechange", a);
...
(function() {
	Fx.Scroll = new Class({
		...
		calculateScroll : function(g, f) {
			var d = this.element, b = d.getScrollSize(), h = d.getScroll(), j = d
					.getSize(), c = this.options.offset, i = {
				x : g,
				y : f
			};
			for (var e in i) {
		...
		toBottom : function() {
			return this.start
					.apply(this, this.calculateScroll(false, "bottom"))
		},
		toElement : function(d, e) {
			...
			var b = Object.map(document.id(d).getPosition(this.element),
					function(g, f) {
		...
		toElementEdge : function(d, g, e) {
			...
			d = document.id(d);
			var i = {}, f = d.getPosition(this.element), j = d.getSize(), h = this.element
					.getScroll(), b = this.element.getSize(), c = {
				x : f.x + j.x,
				y : f.y + j.y
			};
			["x", "y"].each(function(k) {
		...
		toElementCenter : function(e, f, h) {
			...
			e = document.id(e);
			var i = {}, c = e.getPosition(this.element), d = e.getSize(), b = this.element
					.getScroll(), g = this.element.getSize();
			["x", "y"].each(function(j) {
						if (f.contains(j)) {
							i[j] = c[j] - (g[j] - d[j]) / 2
						}
...
(function(b) {
	...
	K.applyWidgets = function(d) {
		var c = "data-widget";
		d = d || document;
		if (!d) {
			return
		}
		d.getElements("[" + c + "]").each(function(g) {
					var f = g.get(c);
					if (f) {
						f = f.split(" ")
					} else {
	...
	K.widgets.backtop = (function() {
		var c = false;
		function d() {
			var e = new Element("div", {
						html : '<img src="/static/ico/top.gif" title="返回顶部" />'
					}).setStyles({
						position : "fixed",
						right : 20,
						bottom : 40,
						cursor : "pointer"
					}).inject(b(document.body)).fade("hide");
			if (Browser.ie6) {
				return
			}
